    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 transition-colors duration-500 flex flex-col">
      ...
      <div className="flex-grow flex items-center justify-center px-4 py-20 relative overflow-hidden">
        ...
        <div className="w-full max-w-md bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200/50 dark:border-slate-800/80 shadow-2xl p-8 md:p-10 rounded-[2rem] transition-all duration-300">
          ...
          {error && (
            <div className="mb-6 p-4 bg-rose-50 dark:bg-rose-950/30 border border-rose-200/55 dark:border-rose-900/40 text-rose-600 dark:text-rose-400 text-sm rounded-2xl animate-shake">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label 
                  htmlFor="password" 
                  className="block text-xs uppercase tracking-wider font-bold text-slate-500 dark:text-slate-400"
                >
                  Password
                </label>
              </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold rounded-2xl shadow-lg shadow-blue-500/10 hover:shadow-xl hover:shadow-blue-500/25 transition-all duration-300 transform active:scale-[0.98] focus:outline-none flex items-center justify-center gap-2 disabled:opacity-80"
            >
              {isLoading ? (
                <>
